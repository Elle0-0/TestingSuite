def find_optimal_route(cost_matrix: list[list[int]]) -> tuple[list[int], int]:
    ...
    # Number of nodes excluding 0
    num_nodes = n - 1
    full_mask = (1 << num_nodes) - 1
    
    # dp[mask][i] where i is 0-indexed among nodes 1..n-1
    ...
    # Initialize: starting from node 0, go to each node i
    ...
    current = last_node
    while current != -1:
        route_reversed.append(current + 1)  # Convert back to original indexing
        prev = parent[mask][current]
        mask = mask ^ (1 << current)
        current = prev
    
    route_reversed.reverse()
    route = [0] + route_reversed + [0]
