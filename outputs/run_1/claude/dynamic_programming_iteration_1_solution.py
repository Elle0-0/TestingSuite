def find_optimal_route(cost_matrix: list[list[int]]) -> tuple[list[int], int]:
    ...
    # Held-Karp algorithm using dynamic programming
    # dp[mask][i] = minimum cost to reach node i, having visited the set of nodes represented by mask
    # mask is a bitmask of visited nodes (excluding node 0 which is always the start)
    # Nodes 1..n-1 are represented in the bitmask
    
    num_nodes = n - 1  # nodes other than 0
    full_mask = (1 << num_nodes) - 1
    
    INF = float('inf')
    
    # dp[mask][i] where i is index in 0..num_nodes-1 (representing nodes 1..n-1)
    # mask represents which of nodes 1..n-1 have been visited
    dp = [[INF] * num_nodes for _ in range(1 << num_nodes)]
    parent = [[-1] * num_nodes for _ in range(1 << num_nodes)]
    
    # Base case: start at node 0, go to node i+1
    ...
    for mask in range(1, 1 << num_nodes):
        for last in range(num_nodes):
            ...
            if dp[mask][last] == INF:
                continue
            # Try extending to an unvisited node
    ...
    current = last_node
    while current != -1:
        route.append(current + 1)  # Convert back to original node index
